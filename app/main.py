...
def chat(
    ...
    # Mock LLM đã trả chi phí tất định; chốt ngân sách nguyên tử trước khi lưu
    # response. Với LLM thật, bước tương đương nên reserve chi phí ước lượng
    # trước khi gọi provider rồi đối soát lại theo usage thực tế.
    guard.check_and_record(client_id, result["usd_cost"])
    if isinstance(store, ChatStore):
        store.add_exchange(client_id, payload.message, result["text"])
    else:
        # Giữ giao diện store tối thiểu để dependency override/stub vẫn dùng được.
